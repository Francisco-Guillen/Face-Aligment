...
def calculate_roc_curve(y_true, y_score, thresholds):
...
def lfw_evaluation_protocol(embedings_a, embedings_b, matches, l2_norm=True, dist='cosine'):
    ...
    """"
    Compares the embeddings of LFW pairs using the 10-fold standard protocol

    embedings_a - 6000xD matrix containing the features of the first image of each LFW pair
    embedings_a - 6000xD matrix containing the features of the second image of each LFW pair
    matches     - 6000x1 array containing if the pair is a match or mis-match pair

    l2_norm     - boolean (True if the embedings should be normalized to have l2 norm equal to 1)
    dist        - metric used to compare the embedings {'euclidean', 'cosine'}
    """


    nfolds = 10  # predefined by LFW protocol
    ...
    folds = [list(range(595 * i, 595 * (i + 1))) for i in range(10)]
    ...
    for i in range(10):
        ...
        fpr_train[i], tpr_train[i] = calculate_roc_curve(matches_train, scores_train, thresholds)
        ...
        fpr_test[i], tpr_test[i] = calculate_roc_curve(matches_test, scores_test, thresholds)
        auc_test[i] = roc_auc_score(matches_test, scores_test)

        true_predicts = torch.sum((scores_test > optim_th) == matches_test)
        accuracy[i] = true_predicts.item()*100 / float(len(test_idx))
    ...
    avg_accuracy = np.round(np.mean(accuracy), 3)
